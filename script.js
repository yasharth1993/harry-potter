...
if(trueFalse=='true' && localStorage.user){
  ...
  document.querySelector('.change-email').value = storedData[0].email;
...
const charactersTypes = function(types){
  fetch(`https://hp-api.onrender.com/api/characters/house/${types}`)
  .then(res=> {return res.json()})
  .then(data=>{
    data.forEach(function (charData) {
      const elImg = document.createElement("img");
...
function move(event){
  ...
  document.getElementsByClassName('bg')[0].style.setProperty('--X' ,X +'px')
  document.getElementsByClassName('bg')[0].style.setProperty('--Y' ,Y +'px')
}
